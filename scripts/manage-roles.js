...
try {
  switch (cmd) {
    ...
    case 'set-heads': {
      // Set the department heads (主管) for a role. They may manage the role's
      // folders even when the role is read-only for ordinary members.
      // npm run role set-heads <角色名> [用户名 ...]   (无用户名表示清空)
      if (!name) throw new Error('用法：npm run role set-heads <角色名> [用户名 ...]');
      const role = updateRole(name, { leaders: rest });
      console.log(`✓ 已将角色“${name}”的部门主管设为：${role.leaders.join('、') || '（无）'}`);
      break;
    }
    case 'add-folder': {
    ...
    case 'ls': {
      ...
      if (roles.length === 0) {
        ...
        for (const r of roles) {
          ...
          if (r.leaders && r.leaders.length) console.log(`      部门主管：${r.leaders.join('、')}`);
        }
      }
      break;
    }
    default:
      ...
      console.log('  npm run role set-heads <角色名> [用户名 ...]   设置部门主管（可管理该角色文件夹）');
